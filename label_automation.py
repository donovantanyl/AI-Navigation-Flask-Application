...
Usage - sources:
    $ python main.py --weights yolov5s.pt --source 0                               # webcam
                                                     img.jpg                         # image
                                                     vid.mp4                         # video
                                                     screen                          # screenshot
                                                     path/                           # directory
...
Usage - formats:
    $ python main.py --weights yolov5s.pt                 # PyTorch
...
from classes.coco_json import Annotation, COCO, Image
import os
import platform
import sys
...
def run(
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        # [WK] Boolean variable to check if a bus is detected
        is_bus = False
        image_count = 0
        label_count = 0
        images = []
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det):
                ...
                # [WK] Set image path and initialize list of json
                image_id = uuid4().hex
                list_json = []
                image = Image(image_count, image_id + ".jpg", r_im0_shape[0], r_im0_shape[1])
                images.append(image.return_json())
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    label_count += 1
                    list_json.append(annotation.return_json())
                ...
                # [WK] Checks and saves frames with buses
                if is_bus:
